...
const OnBoardingStep05 = ({ nextStep }) => {
  
  // 사용자가 선택한 장르를 저장하는 상태
  const [selectedGenres, setSelectedGenres] = useState([]);

  // 선택된 장르가 변경될 때마다 콘솔에 출력 (디버깅 목적)
  useEffect(() => {
  ...
  // 선택 가능한 장르 목록
  const genres = ['소설/문학', '판타지', 'SF', '미스터리', '에세이/시', '자기계발', '사회/경제', '인문/철학', '심리학'];
  ...
  return (
    <div className={styles.wrap}>
      ...
      <div className={styles.contents}>
        <div className={styles.content}>
          ...
          <div className={styles.subtitle}>
            <Title style={{ fontSize: '20px', fontWeight: '600' }}>나의 선호하는 장르를 선택해주세요!</Title>
            <Title style={{ fontSize: '14px', fontWeight: '400' }}>최대 2개까지 선택 가능합니다.</Title>
          </div>
        </div>
      ...
      <Button className={styles.Button} onClick={handleNextStep} disabled={selectedGenres.length === 0}>
        다음
      </Button>
    </div>
  );
